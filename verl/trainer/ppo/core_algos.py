...
@register_adv_est(AdvantageEstimator.LOOP)
...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str):
    ...
    elif loss_agg_mode == "seq-mean-token-sum-length-norm": # real DR.GRPO
        seq_losses = torch.sum(loss_mat * loss_mask, dim=-1) / loss_mat.shape[-1]  # token-sum w/ length-norm
        loss = torch.mean(seq_losses)  # seq-mean
    else:
